...
def compute_ARI(conf_mat: NDArray[np.int32]):
    """
    ...
    Parameters:
    conf_mat (numpy.ndarray): The confusion matrix representing the clustering results.
...
def confusion_matrix(true_labels, predicted_labels):
    ...
    for true, pred in zip(true_labels, predicted_labels):
        conf_mat[class_index[true]][class_index[pred]] += 1
...
def gaussian_mixture():
    ...
    plt.title("Ground truth for 10000 samples")
    plt.show()

    weights, means, covariances, log_likelihoods, confusion_mat, ARI, SSE = sample_estimate(
        data, labels
    )
    ...
    answers["log_likelihoods"] = log_likelihoods
    answers["confusion_matrix"] = confusion_mat
    answers["ARI"] = ARI
    answers["SSE"] = SSE
    # print(f"{confusion_mat=}, {ARI=}")
    # Save the results in a pickle file
    with open("question2_em_gmm_results.pickle", "wb") as f:
